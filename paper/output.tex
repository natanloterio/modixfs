...
the protocol layer that POSIX-based systems structurally lack. A
secondary finding is the gap between claimed and implemented behavior:
ToolFS's documented WASM sandbox is an unimplemented stub. All
...
\textbf{05 Security.} This criterion reveals the sharpest divergence in
the survey. MCP rates ✓ (strong): input schemas are enforced before any
...
isolation provides OS-enforced boundaries. LiveFoldersFS rates ✗ (weak):
shell handlers receive raw input, and injection is possible if the
handler does not sanitize. llm9p rates ✗ for a distinct and more severe
reason --- by default, it binds an unauthenticated TCP port, making the
tool surface accessible to any process on the network. This is the worst
security finding in the survey and represents a deployment risk in
multi-tenant or networked environments. ToolFS and AgentFS rate
...
routes stderr to a log file and returns errors as plain text to the LLM
--- functional but unstructured. llm9p rates ✗ (weak): its architectural
inversion means there is no clear place in its design for tool execution
observability.
...
not the application layer. A poorly written LiveFoldersFS handler may
execute shell commands with unsanitized input, creating injection
vulnerabilities. This difference is not a matter of practitioner skill
--- it is structural. The posix invocation model concentrates security
responsibility entirely on the individual handler author; the rpc
invocation model distributes it across the protocol and the handler.
...
\emph{Schema inference} would automatically derive JSON Schema from
shell handler signatures, bridging the posix and rpc invocation models.
If a LiveFoldersFS handler declares its expected inputs via a
conventions-based DSL (similar to argparse or docopt), a schema can be
generated and enforced without requiring the author to write it
manually.
...
security, observability, and portability. No current system fully
...
abstraction's practical appeal; the security gap relative to
