...
tiers: live experiments for LiveFoldersFS and MCP, structured assessment
for ToolFS, AgentFS, and llm9p, and paper-only assessment for InferNode
and Quine. Our central finding is that coupling depth predicts
ergonomics ceiling while invocation interface predicts safety floor, and
no current system optimizes both simultaneously. Concretely,
LiveFoldersFS requires 10 lines of YAML versus MCP's 18 lines of Python
for an equivalent REST tool --- but MCP enforces schema validation at
the protocol layer while POSIX-based systems must opt in at the tool
level. A secondary finding is the gap between claimed and implemented
...
isolation (MCP via separate server process), and claimed WASM sandboxing
(ToolFS).
...
\begin{figure}
\centering
\includegraphics{figures/taxonomy.svg}
...
available and how to call them? LiveFoldersFS exposes a human-readable
markdown \texttt{index.md} that the LLM reads naturally as a file. MCP
exposes a typed JSON schema via \texttt{list\_tools} that enables
parameter validation and IDE tooling. Both rate \textasciitilde{}
(partial): LiveFoldersFS is readable but lacks parameter types; MCP is
typed but requires the LLM to navigate JSON rather than prose. ToolFS
follows a similar file-listing approach to LiveFoldersFS. AgentFS and
llm9p rate --- (N/A): AgentFS is a storage substrate with no
tool-invocation interface to discover, and llm9p's architectural
inversion means there are no tools to discover in the conventional
sense. InferNode and Quine rate \textasciitilde{} based on their
published descriptions of namespace-based discovery.
...
well-suited for multiline text and binary piping, but without schema
enforcement. MCP rates \textasciitilde{} for the same reason: structured
JSON is strong, but multiline text requires string escaping and binary
data requires base64 encoding. The notable outlier is Quine (✓), which
...
\textasciitilde{} (partial): v0.6.0 introduced opt-in per-endpoint input
validation via an \texttt{input:} field in \texttt{folder.yaml} (types:
\texttt{json}, \texttt{string}, \texttt{none}), which rejects malformed
input before the handler runs and returns a structured
\texttt{{[}ERROR:INVALID\_INPUT{]}} response. This closes the validation
gap for well-authored tools; however, the guarantee is opt-in rather
than protocol-enforced --- a handler author must declare the schema for
it to be checked, and shell injection within the handler body remains
the author's responsibility. llm9p rates ✗ for a distinct and more
severe reason --- by default, it binds an unauthenticated TCP port,
making the tool surface accessible to any process on the network. This
is the worst security finding in the survey and represents a deployment
risk in multi-tenant or networked environments. ToolFS and AgentFS rate
\textasciitilde{} (partial); notably, ToolFS's documentation claims WASM
sandbox isolation, but code inspection (T2) reveals this is implemented
as \texttt{InMemorySandbox} --- a stub with no actual isolation. The
security claim in ToolFS's documentation does not reflect the current
implementation.

\textbf{06 Stateful tools.} How well does each system support tools that
maintain state across invocations? ToolFS rates ✓ (strong), with
explicit support for in-process Go plugin state. MCP and LiveFoldersFS
both rate \textasciitilde{} (partial) for complementary reasons: MCP
holds state in Python process memory (fast, but lost on server restart),
while LiveFoldersFS persists state to files (durable, but concurrent
writes require external locking). InferNode and Quine rate
\textasciitilde{} based on their published descriptions of kernel-level
state management. No surveyed system provides both durability and
transactional consistency for tool state without additional
infrastructure.

\textbf{07 Composability.} This criterion measures how easily tool
outputs can be chained or combined. Quine and InferNode rate ✓ (strong),
leveraging Inferno's per-process namespaces to compose tool environments
hierarchically --- nested agents receive composed views without
coordination overhead. MCP and LiveFoldersFS both rate \textasciitilde{}
(partial): MCP enables clean Python function composition within a
server, but cross-server tool chaining requires LLM orchestration;
LiveFoldersFS enables shell pipeline composition with standard Unix
tools, but cross-tool calls involve awkward file-endpoint writes.
AgentFS rates --- (N/A) as a storage substrate.

\textbf{08 Observability.} How well does the system support diagnosing
errors and monitoring tool execution? MCP rates ✓ (strong): Python
exceptions are automatically converted to structured error objects with
type, message, and traceback; the LLM receives machine-parseable error
information. AgentFS rates ✓ (strong) --- it is purpose-built as an
observability substrate, providing structured audit logs and execution
traces as first-class filesystem artifacts. However, 4 of 10 criteria
rate --- (N/A) for AgentFS because it is a storage and observation
layer, not a tool-invocation interface in its own right. LiveFoldersFS
rates \textasciitilde{} (partial): v0.6.0 standardized all error returns
to the format \texttt{{[}ERROR:CODE{]}\ message} with five
machine-parseable codes (\texttt{HANDLER}, \texttt{TIMEOUT},
\texttt{SPAWN}, \texttt{PROCESS}, \texttt{INVALID\_INPUT}), and the
auto-generated \texttt{how\_to.md} now documents each endpoint's
declared input type. This gives LLMs and scripts a structured signal to
act on. The gap relative to MCP's ✓ is that LiveFoldersFS does not
capture stdout/stderr tracing or execution timing --- errors are
reported, not traced. llm9p rates ✗ (weak): its architectural inversion
means there is no clear place in its design for tool execution
observability.
...
author. LiveFoldersFS v0.6.0 narrows this gap through opt-in
per-endpoint validation: adding \texttt{input:\ type:\ json} to a
\texttt{folder.yaml} endpoint causes the runtime to reject non-JSON
input before the handler runs, returning
\texttt{{[}ERROR:INVALID\_INPUT{]}} without executing any shell code.
This represents a meaningful improvement --- tool authors can now opt
into schema enforcement without abandoning the posix authoring model.
The remaining gap is structural: MCP validation is enforced
unconditionally at the protocol layer, while LiveFoldersFS validation
requires explicit opt-in per endpoint.
...
\emph{VFS-layer sandboxing} would combine os-coupling's ergonomic
advantages with rpc-coupling's security guarantees. A FUSE filesystem
that executes handlers inside Linux namespaces or seccomp-BPF sandboxes
could offer POSIX authoring convenience with process-level isolation
stronger than what the current generation of os-coupled systems
provides. ToolFS gestures at this direction with its WASM sandbox
design; the implementation gap we documented (T2) represents an
opportunity.
...
\emph{Richer schema enforcement} would extend the opt-in input
validation introduced in LiveFoldersFS v0.6.0 (types: \texttt{json},
\texttt{string}, \texttt{none}) to structural constraints:
minimum/maximum length, regex patterns, and JSON Schema shapes. The
current \texttt{input:} field declares input \emph{kind} but not
\emph{structure} --- a handler declared as \texttt{type:\ json} still
receives arbitrary JSON. Adding structural constraints would bring
posix-invocation systems closer to the safety guarantees that RPC
systems provide at the protocol layer, without abandoning the YAML
authoring model.
...
a structured \texttt{{[}ERROR:CODE{]}} error format, improving its
security and observability ratings from ✗ to \textasciitilde{} and
demonstrating that posix-invocation systems can add schema enforcement
without abandoning their authoring advantages. No current system fully
optimizes both dimensions. The independent convergence of multiple teams
on filesystem abstractions for LLM tool integration validates the
