...
behavior: ToolFS's documented WASM sandbox is an unimplemented stub. All
...
Traces are logged to Weights \& Biases Weave for reproducibility.
...
LF-manifest is within ±8\% depending on task complexity.}
\end{figure}
...
+2--8\% of MCP. The original bash backend is 2--8× more expensive and
excluded from this table for space.}
...
is 155--400\% more expensive than MCP depending on task: it generates
multi-step bash pipelines in output, consumes full tool-result text in
subsequent turns, and sometimes fails to complete within the 10-turn
safety cap. It is not suitable for production token-sensitive
deployments.
...
The token efficiency benchmark (§3.6) adds a quantitative dimension to
this principle. When LiveFoldersFS exposes tools as Anthropic-native
schemas via \texttt{anthropic\_tools.json}, it achieves 6--10\% lower
token consumption than MCP across all 8 benchmark tasks. The advantage
