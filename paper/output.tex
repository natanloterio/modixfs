...
MCP's 18 lines of Python for an equivalent REST tool. A third finding is
that LiveFoldersFS's native tool schema approach --- Anthropic-format
tool definitions synthesized at read time from \texttt{folder.yaml} via
\texttt{anthropic\_tools.json} --- achieves 6--10\% lower token
consumption than MCP across an 8-task benchmark suite covering simple
reads, pipe composition, multi-tool sequences, aggregate/filter, and
cross-reference tasks, with 100\% task completion on all tasks and all
backends. A secondary finding is the gap between claimed and implemented
behavior: ToolFS's documented WASM sandbox was found to be an
unimplemented stub at the time of T2 code inspection (early 2025). All
experiments are reproducible via the public LiveFoldersFS repository.
\end{abstract}
...
\hypertarget{livefoldersfs-architecture}{%
\subsection{LiveFoldersFS
Architecture}\label{livefoldersfs-architecture}}

Figure 2 shows the internal architecture of LiveFoldersFS, the primary
T1 system in our evaluation. The design reflects its os-coupled,
posix-invocation taxonomy placement: the FUSE kernel layer is the sole
entry point, and every tool invocation reduces to a standard file write
followed by a file read --- no protocol, no SDK.

The central component is \texttt{src/fs/vfs.rs}, which implements the
\texttt{Filesystem} trait from the \texttt{fuser} crate. It maintains
three in-memory maps keyed by inode: \texttt{write\_buf} accumulates
bytes across successive \texttt{write()} calls; \texttt{result\_buf}
stores the handler output after invocation; \texttt{trace\_buf} holds
the companion \texttt{.log} content with execution timing and stderr.
The FUSE \texttt{release()} event --- fired when the LLM closes the file
descriptor --- triggers handler dispatch for \texttt{write\_invoke}
endpoints; \texttt{read\_invoke} endpoints fire on \texttt{read()}
instead.

Tool dispatch flows through \texttt{src/registry/}, which maps names to
\texttt{Arc\textless{}dyn\ Tool\textgreater{}} implementations. Built-in
tools are registered at startup; external tools are loaded from
subdirectories under \texttt{tools\_dir}, each described by a
\texttt{folder.yaml} manifest parsed by \texttt{src/manifest.rs}. The
manifest declares endpoint types, input schemas, optional
\texttt{state\_file} paths for serialised concurrent access, and
optional \texttt{sandbox} policies. Two virtual files ---
\texttt{how\_to.md} and \texttt{schema.json} --- are synthesised at read
time by \texttt{src/fs/how\_to\_gen.rs} and
\texttt{src/fs/schema\_gen.rs}; they are never written to disk.

Process isolation is provided by \texttt{src/sandbox/}, which applies
\texttt{PR\_SET\_NO\_NEW\_PRIVS}, Landlock filesystem access control,
and seccomp-BPF network filtering before each handler process starts. An
\texttt{inotify}-based watcher monitors \texttt{tools\_dir} for changes
and hot-reloads the registry without remounting. The daemon layer forks
after mount options are validated, redirecting stderr to a log file and
writing a PID file for the \texttt{stop} subcommand.

\begin{figure}
\centering
\includegraphics{figures/architecture.pdf}
\caption{LiveFoldersFS internal architecture. Arrows show the dispatch
path from LLM I/O through the FUSE layer to tool handlers; dashed arrows
indicate asynchronous or event-driven paths.}
\end{figure}
...
without a compatibility shim. LiveFoldersFS partially bridges this gap
via \texttt{anthropic\_tools.json}, a virtual file synthesized at read
time from \texttt{folder.yaml} that exposes each invokable endpoint as
an Anthropic-format native tool definition. Clients that can read this
file --- including any agent with filesystem access --- can inject the
definitions directly into the \texttt{tools={[}{]}} API parameter,
enabling structured tool calling without shell access. This approach is
evaluated empirically in the
\protect\hyperlink{token-efficiency-benchmark}{Token Efficiency
Benchmark} below.
...
inspection (T2, assessed against the repository main branch as of early
2025) reveals this is implemented as \texttt{InMemorySandbox} --- a stub
with no actual isolation. The security claim in ToolFS's documentation
does not reflect the implementation examined; subsequent releases may
differ.
...
\hypertarget{token-efficiency-benchmark}{%
\subsection{Token Efficiency
Benchmark}\label{token-efficiency-benchmark}}

Token consumption is a direct proxy for inference cost and latency in
deployed LLM agents. Every tool schema injected into a request, every
tool-call result returned, and every model output turn contributes to
the token budget. We measure this empirically by running identical tasks
against LiveFoldersFS and MCP and recording per-turn token usage via the
Anthropic API.

\textbf{Benchmark design.} We define five backends over the same mock
REST API (50 users, mockapi.io):

\begin{itemize}
\tightlist
\item
  \emph{livefolders} (baseline): original bash invocation via
  \texttt{cat}/\texttt{echo}, system prompt generated from
  \texttt{system\_prompt.md}.
\item
  \emph{livefolders-manifest}: same filesystem, but the system prompt is
  the concise per-endpoint listing synthesized by the VFS from
  \texttt{folder.yaml}.
\item
  \emph{livefolders-native}: tool schemas loaded from
  \texttt{anthropic\_tools.json}; the model calls structured Anthropic
  tools (\texttt{users\_\_count}, \texttt{users\_\_search}, etc.) and
  the benchmark executes the corresponding file operations.
\item
  \emph{livefolders-unified}: \texttt{anthropic\_tools.json} schemas
  collapsed into one tool per namespace with an \texttt{action} enum,
  reducing schema size at the cost of per-action guidance.
\item
  \emph{mcp} (baseline): standard MCP server over stdio; tool schemas
  injected via the MCP \texttt{list\_tools} response.
\end{itemize}

All five backends call the same upstream API endpoint. Tasks are run 5
times each; results are averaged. The model is claude-haiku-4-5 (the
lowest-cost Anthropic model, maximizing sensitivity to schema overhead).
Traces are logged to Weights \& Biases Weave for reproducibility. Note
that mockapi.io is a third-party service; if the endpoint becomes
unavailable, researchers should substitute a locally-hosted mock with
equivalent schema and data volume.

\textbf{Task suite.} We evaluate 8 tasks covering the breadth of real
agent workloads:
...
\midrule
\endhead
\texttt{count\_users} & Count total users & single read \\
\texttt{count\_composed} & Count via pipe-composed endpoint & single
read (server-side pipe) \\
\texttt{list\_users} & List all users with name and ID & single read \\
\texttt{list\_compact} & List users as compact id:name pairs & single
read (server-side pipe) \\
\texttt{single\_user} & Look up a user by name & single write+read \\
\texttt{count\_and\_find} & Count users AND find Brad Jacobi's ID & two
tool calls \\
\texttt{filter\_and\_count} & Find all users with `a' in name; count
them & write+read (server-side filter) \\
\texttt{cross\_reference} & Find Brad Jacobi's account creation date &
two tool calls: search → get-by-id \\
\bottomrule
\end{longtable}

\textbf{Results.} Figure 3 shows token consumption per task for the four
primary backends; Figure 4 shows the percentage delta versus MCP. Table
3 gives the numerical values. All optimised backends achieved 100\% task
completion across all 5 runs. With only 5 runs per task we do not report
confidence intervals; margins smaller than approximately 3\% should be
treated as indicative rather than statistically robust.

\begin{figure}
\centering
\includegraphics{figures/token_absolute.pdf}
\caption{Token consumption per task and backend (5-run averages). Lower
is better. The MCP baseline (red) is consistently above both LF-native
and LF-unified across all 8 tasks.}
\end{figure}

\begin{figure}
\centering
\includegraphics{figures/token_delta.pdf}
\caption{Token efficiency relative to MCP. Negative values mean fewer
tokens than MCP. LF-native (dark blue) beats MCP on every task;
LF-manifest is within +2--8\% of MCP (i.e., consistently more expensive
than MCP) on most tasks.}
\end{figure}
...
\midrule
\endhead
\texttt{count\_users} & 1,893 & 1,915 & 2,094 & −9.6\% \\
\texttt{count\_composed} & 1,892 & 1,914 & 2,093 & −9.6\% \\
\texttt{single\_user} & 1,962 & 2,235 & 2,163 & −9.3\% \\
\texttt{list\_users} & 2,917 & 2,915 & 3,176 & −8.2\% \\
\texttt{cross\_reference} & 3,190 & 3,231 & 3,471 & −8.1\% \\
\texttt{count\_and\_find} & 2,102 & 2,148 & 2,282 & −7.9\% \\
\texttt{filter\_and\_count} & 2,582 & 2,622 & 2,794 & −7.6\% \\
\texttt{list\_compact} & 2,786 & 2,805 & 2,975 & −6.4\% \\
\bottomrule
\end{longtable}

\emph{Table 3: Average token consumption per task (5 runs). LF-native
and LF-unified both beat MCP on every task. LF-manifest is within
+2--8\% of MCP (consistently worse). The original bash backend is
2.5--5× more expensive on completing runs (up to 8× on near-timeout
runs) and excluded from this table for space.}

\textbf{Analysis.} The livefolders-native backend outperforms MCP by
6--10\% on every task. The margin is consistent across task complexity,
from single-call reads to two-step cross-reference lookups. The
mechanism is straightforward: LiveFoldersFS's per-tool schema is compact
relative to MCP's tool list because hidden endpoints (marked
\texttt{hidden:\ true} in \texttt{folder.yaml}) are excluded from
\texttt{anthropic\_tools.json}, while MCP exposes all registered tools
unconditionally. For the users API with 6 visible endpoints and 3 hidden
internal stages, the LF schema is approximately 15\% smaller than the
equivalent MCP \texttt{list\_tools} response.

Server-side composition amplifies this advantage on aggregate tasks. The
\texttt{filter\_and\_count} task (``how many users have `a' in their
name?'') initially regressed against MCP (+0.1\% for livefolders-native)
before a \texttt{filter} endpoint was added to \texttt{folder.yaml} ---
a write-invoke handler that runs the substring match server-side and
returns \texttt{count:\ N} followed by matching \texttt{id:name} pairs
in a single tool call. After adding \texttt{filter}, livefolders-native
moved to −7.6\% vs MCP, with output tokens dropping from
\textasciitilde963 to \textasciitilde386 because the model receives a
pre-aggregated answer rather than all 50 users for in-context filtering.
The same principle applies to the \texttt{cross\_reference} task: adding
a \texttt{get} endpoint (fetch-by-ID) reduces output tokens because the
model receives exactly the fields it needs (\texttt{name}, \texttt{id},
\texttt{createdAt}, \texttt{avatar}) rather than scanning a full
listing.

The livefolders-unified backend trades per-action description richness
for a smaller schema and performs 2--5\% worse than livefolders-native
overall, with a notable regression on \texttt{single\_user} (+3.3\%)
where the action-enum format provides insufficient guidance for
single-endpoint selection. The livefolders-manifest backend (bash-style)
remains within +2--8\% of MCP --- a large improvement over the original
bash backend but behind the structured tool approaches.

The original bash backend (\texttt{livefolders}, not shown in Table 3)
is 155--400\% more expensive than MCP on tasks that complete (2.5--5×
total); runs that approach the 10-turn safety cap push the ratio toward
the 8× upper bound cited in Table 3. It generates multi-step bash
pipelines in output, consumes full tool-result text in subsequent turns,
and sometimes fails to complete within the safety cap. It is not
suitable for production token-sensitive deployments.
...
Foundation in December 2025 (Anthropic 2024), signaling institutional
commitment to its longevity as an open standard. OpenAI's function
calling and LangChain's tool abstractions operate at the SDK layer over
conceptually similar RPC models, though they are not interoperable with
MCP at the protocol level.
...
Several factors bound the generalisability of our findings. First, the
token efficiency benchmark uses a single model (claude-haiku-4-5) and a
single upstream API (mockapi.io, 50 users); token margins may shift for
larger models, denser APIs, or workloads with different
tool-call-to-output ratios. Second, T2 and T3 assessments capture a
point-in-time code and documentation snapshot; rapidly evolving projects
may have addressed noted deficiencies since inspection. Third, with only
5 runs per task, the benchmark does not support formal statistical
inference; any effect below \textasciitilde3\% should be treated as
indicative. Fourth, the evaluation host (Claude Code on Linux x86\_64
with shell access) may not represent cloud-hosted agent deployments
where the LLM has no filesystem access, which would lower the LLM
compatibility ratings for all os-coupled systems.
...
(§\protect\hyperlink{token-efficiency-benchmark}{Token Efficiency
Benchmark}) adds a quantitative dimension to this principle. When
LiveFoldersFS exposes tools as Anthropic-native schemas via
\texttt{anthropic\_tools.json}, it achieves 6--10\% lower token
consumption than MCP across all 8 benchmark tasks. The advantage
compounds on aggregate and cross-reference tasks because server-side
composition --- pipe chains, filter endpoints, get-by-ID handlers ---
moves work from model context to tool execution, reducing both output
tokens and in-context reasoning load. This is structurally unavailable
to protocol-decoupled systems without equivalent server-side
aggregation, because they lack the POSIX pipeline model that makes
composing handlers trivial.
...
running on remote hosts to agents anywhere. The
\texttt{anthropic\_tools.json} approach partially bridges this gap: a
client that can read the file can use structured tool calling without
shell access, though the tool execution layer still requires filesystem
access on the mount host.
...
portability and unconditional schema enforcement. A new quantitative
finding from the token efficiency benchmark extends this picture:
LiveFoldersFS's native tool schema approach achieves 6--10\% lower token
consumption than MCP across an 8-task suite at 100\% task completion,
with server-side composition (pipe chains, aggregate filter endpoints,
get-by-ID handlers) providing the largest per-task gains on multi-step
workloads. The gap is narrowing: LiveFoldersFS v0.6.0 introduced opt-in
per-endpoint input validation and a structured \texttt{{[}ERROR:CODE{]}}
error format; v0.7.0 extends this to structural constraints --- string
endpoints support \texttt{min\_length}, \texttt{max\_length}, and regex
\texttt{pattern}; JSON endpoints support a \texttt{schema:} sub-key with
required-field and property-type enforcement; v0.8.0 adds
machine-readable \texttt{schema.json} discovery (criterion 03 ✓),
durable stateful locking via \texttt{state\_file} + \texttt{flock}
(criterion 06 ✓), and per-invocation \texttt{.log} files with timing and
stderr (criterion 08 ✓); v0.9.0 adds declarative \texttt{pipe:}
multi-stage chaining (criterion 07 ✓), making LiveFoldersFS the only
non-Inferno system to achieve ✓ on composability; v0.11.0 implements
opt-in VFS-layer sandboxing via Landlock and seccomp-BPF (criterion 05
✓). No current system fully optimizes both dimensions simultaneously
across all criteria. The independent convergence of multiple teams on
filesystem abstractions for LLM tool integration validates the
abstraction's practical appeal; the remaining safety gap relative to
protocol-based systems explains why protocol approaches have gained
institutional traction.

The most striking finding from our Tier 2 assessment is the gap between
claimed and implemented behavior: ToolFS's WASM sandbox was an
unimplemented stub at the time of code inspection (early 2025); llm9p's
unauthenticated TCP transport creates network-accessible tool surfaces.
Practitioners evaluating these systems should treat published claims
about security properties as hypotheses pending independent verification
...
The author is the primary developer of LiveFoldersFS, one of the two T1
systems evaluated in this paper. The use of explicit evidence tiers and
publicly reproducible experiments is intended to mitigate, though not
eliminate, the risk of confirmation bias. All T1 results are
reproducible from the repository root via
\texttt{bash\ research/run-all.sh}.
