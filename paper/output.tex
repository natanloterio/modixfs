...
level. LiveFoldersFS v0.7.0 narrows this gap through richer structural
validation: the \texttt{input:} field now supports \texttt{min\_length},
\texttt{max\_length}, and \texttt{pattern} constraints for string
endpoints, and a \texttt{schema:} sub-key accepting a JSON Schema subset
(required fields, property types) for JSON endpoints; all constraints
are rejected before any handler executes and surfaced inline in
auto-generated \texttt{how\_to.md}. LiveFoldersFS v0.8.0 further
advances three criteria to ✓: discoverability via a machine-readable
\texttt{schema.json} mirroring MCP's \texttt{list\_tools} format;
stateful tools via a \texttt{state\_file} manifest field with runtime
\texttt{flock(LOCK\_EX)} locking and \texttt{LIVEFOLDERS\_STATE\_FILE}
injection; and observability via per-endpoint
\texttt{\textless{}endpoint\textgreater{}.log} files recording execution
timing and stderr after each invocation. LiveFoldersFS v0.9.0 adds a
\texttt{pipe:} field for declarative multi-stage pipelines (criterion 07
✓): a single write invocation chains an ordered list of endpoints,
passing each stage's stdout as the next stage's stdin with per-stage
schema validation and structured error propagation. LiveFoldersFS
v0.11.0 implements opt-in VFS-layer sandboxing on Linux (criterion 05
✓): handlers execute with \texttt{PR\_SET\_NO\_NEW\_PRIVS}, Landlock LSM
filesystem access control (configurable read/write path allowlists),
seccomp-BPF network isolation, and configurable
\texttt{RLIMIT\_NPROC}/\texttt{RLIMIT\_AS} resource limits; per-tool
policy is declared in a \texttt{sandbox:} block in \texttt{folder.yaml}
and global enforcement is controlled by \texttt{sandbox\_mode} in
\texttt{livefolders.yaml}. A secondary finding is the gap between
claimed and implemented behavior: ToolFS's documented WASM sandbox is an
unimplemented stub. All experiments are reproducible via the public
