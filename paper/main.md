...
abstract: |
  ...
  POSIX-based systems must opt in at the tool level. A secondary finding is the gap between claimed
...
**03 Discoverability.** How does an LLM learn what tools are available and how to call them? LiveFoldersFS exposes a human-readable markdown `index.md` that the LLM reads naturally as a file. MCP exposes a typed JSON schema via `list_tools` that enables parameter validation and IDE tooling. Both rate ~ (partial): LiveFoldersFS is readable but lacks parameter types; MCP is typed but requires the LLM to navigate JSON rather than prose. ToolFS follows a similar file-listing approach to LiveFoldersFS. AgentFS and llm9p rate — (N/A): AgentFS is a storage substrate with no tool-invocation interface to discover, and llm9p's architectural inversion means there are no tools to discover in the conventional sense. InferNode and Quine rate ~ based on their published descriptions of namespace-based discovery.

**04 I/O expressiveness.** What data types can a tool consume and produce? All systems handle plain text and JSON strings. LiveFoldersFS rates ~ because it passes raw stdin to handlers — well-suited for multiline text and binary piping, but without schema enforcement. MCP rates ~ for the same reason: structured JSON is strong, but multiline text requires string escaping and binary data requires base64 encoding. The notable outlier is Quine (✓), which operates at the kernel level and can pass arbitrary byte streams through Inferno's typed channels. InferNode similarly benefits from kernel-level data access. No surveyed system simultaneously achieves type safety (MCP's strength) and binary transparency (Quine's strength).

**05 Security.** This criterion reveals sharp divergence in the survey. MCP rates ✓ (strong): input schemas are enforced before any handler executes, rejecting malformed inputs before they reach application code. InferNode rates ✓ at the kernel level, where namespace isolation provides OS-enforced boundaries. LiveFoldersFS rates ~ (partial): v0.6.0 introduced opt-in per-endpoint input validation via an `input:` field in `folder.yaml` (types: `json`, `string`, `none`), which rejects malformed input before the handler runs and returns a structured `[ERROR:INVALID_INPUT]` response. This closes the validation gap for well-authored tools; however, the guarantee is opt-in rather than protocol-enforced — a handler author must declare the schema for it to be checked, and shell injection within the handler body remains the author's responsibility. llm9p rates ✗ for a distinct and more severe reason — by default, it binds an unauthenticated TCP port, making the tool surface accessible to any process on the network. This is the worst security finding in the survey and represents a deployment risk in multi-tenant or networked environments. ToolFS and AgentFS rate ~ (partial); notably, ToolFS's documentation claims WASM sandbox isolation, but code inspection (T2) reveals this is implemented as `InMemorySandbox` — a stub with no actual isolation. The security claim in ToolFS's documentation does not reflect the current implementation.

**06 Stateful tools.** How well does each system support tools that maintain state across invocations? ToolFS rates ✓ (strong), with explicit support for in-process Go plugin state. MCP and LiveFoldersFS both rate ~ (partial) for complementary reasons: MCP holds state in Python process memory (fast, but lost on server restart), while LiveFoldersFS persists state to files (durable, but concurrent writes require external locking). InferNode and Quine rate ~ based on their published descriptions of kernel-level state management. No surveyed system provides both durability and transactional consistency for tool state without additional infrastructure.

**07 Composability.** This criterion measures how easily tool outputs can be chained or combined. Quine and InferNode rate ✓ (strong), leveraging Inferno's per-process namespaces to compose tool environments hierarchically — nested agents receive composed views without coordination overhead. MCP and LiveFoldersFS both rate ~ (partial): MCP enables clean Python function composition within a server, but cross-server tool chaining requires LLM orchestration; LiveFoldersFS enables shell pipeline composition with standard Unix tools, but cross-tool calls involve awkward file-endpoint writes. AgentFS rates — (N/A) as a storage substrate.

**08 Observability.** How well does the system support diagnosing errors and monitoring tool execution? MCP rates ✓ (strong): Python exceptions are automatically converted to structured error objects with type, message, and traceback; the LLM receives machine-parseable error information. AgentFS rates ✓ (strong) — it is purpose-built as an observability substrate, providing structured audit logs and execution traces as first-class filesystem artifacts. However, 4 of 10 criteria rate — (N/A) for AgentFS because it is a storage and observation layer, not a tool-invocation interface in its own right. LiveFoldersFS rates ~ (partial): v0.6.0 standardized all error returns to the format `[ERROR:CODE] message` with five machine-parseable codes (`HANDLER`, `TIMEOUT`, `SPAWN`, `PROCESS`, `INVALID_INPUT`), and the auto-generated `how_to.md` now documents each endpoint's declared input type. This gives LLMs and scripts a structured signal to act on. The gap relative to MCP's ✓ is that LiveFoldersFS does not capture stdout/stderr tracing or execution timing — errors are reported, not traced. llm9p rates ✗ (weak): its architectural inversion means there is no clear place in its design for tool execution observability.
...
RPC systems enforce input schemas before any handler executes. A poorly written MCP tool automatically rejects inputs that do not match its declared parameter schema; the validation happens at the protocol layer, not the application layer. The posix invocation model historically concentrated security responsibility entirely on the individual handler author. LiveFoldersFS v0.6.0 narrows this gap through opt-in per-endpoint validation: adding `input: type: json` to a `folder.yaml` endpoint causes the runtime to reject non-JSON input before the handler runs, returning `[ERROR:INVALID_INPUT]` without executing any shell code. This represents a meaningful improvement — tool authors can now opt into schema enforcement without abandoning the posix authoring model. The remaining gap is structural: MCP validation is enforced unconditionally at the protocol layer, while LiveFoldersFS validation requires explicit opt-in per endpoint.
...
*VFS-layer sandboxing* would combine os-coupling's ergonomic advantages with rpc-coupling's security guarantees. A FUSE filesystem that executes handlers inside Linux namespaces or seccomp-BPF sandboxes could offer POSIX authoring convenience with process-level isolation stronger than what the current generation of os-coupled systems provides. ToolFS gestures at this direction with its WASM sandbox design; the implementation gap we documented (T2) represents an opportunity.
...
*Richer schema enforcement* would extend the opt-in input validation introduced in LiveFoldersFS v0.6.0 (types: `json`, `string`, `none`) to structural constraints: minimum/maximum length, regex patterns, and JSON Schema shapes. The current `input:` field declares input *kind* but not *structure* — a handler declared as `type: json` still receives arbitrary JSON. Adding structural constraints would bring posix-invocation systems closer to the safety guarantees that RPC systems provide at the protocol layer, without abandoning the YAML authoring model.
