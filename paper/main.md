...
RPC systems enforce input schemas before any handler executes. A poorly written MCP tool automatically rejects inputs that do not match its declared parameter schema; the validation happens at the protocol layer, not the application layer. A poorly written LiveFoldersFS handler may execute shell commands with unsanitized input, creating injection vulnerabilities. This difference is not a matter of practitioner skill — it is structural. The posix invocation model concentrates security responsibility entirely on the individual handler author; the rpc invocation model distributes it across the protocol and the handler.
...
*Schema inference* would automatically derive JSON Schema from shell handler signatures, bridging the posix and rpc invocation models. If a LiveFoldersFS handler declares its expected inputs via a conventions-based DSL (similar to argparse or docopt), a schema can be generated and enforced without requiring the author to write it manually.
