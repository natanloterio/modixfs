...
abstract: |
  ...
  documented WASM sandbox was found to be an unimplemented stub at the time of T2 code
  inspection (early 2025). All experiments are reproducible via the public LiveFoldersFS
  repository.
---
...
All five backends call the same upstream API endpoint. Tasks are run 5 times each; results are averaged. The model is claude-haiku-4-5 (the lowest-cost Anthropic model, maximizing sensitivity to schema overhead). Traces are logged to Weights & Biases Weave for reproducibility. Note that mockapi.io is a third-party service; if the endpoint becomes unavailable, researchers should substitute a locally-hosted mock with equivalent schema and data volume.
...
**Results.** Figure 3 shows token consumption per task for the four primary backends; Figure 4 shows the percentage delta versus MCP. Table 3 gives the numerical values. All optimised backends achieved 100% task completion across all 5 runs. With only 5 runs per task we do not report confidence intervals; margins smaller than approximately 3% should be treated as indicative rather than statistically robust.
...
![Token efficiency relative to MCP. Negative values mean fewer tokens than MCP. LF-native (dark blue) beats MCP on every task; LF-manifest is within +2–8% of MCP (i.e., consistently more expensive than MCP) on most tasks.](figures/token_delta.pdf)
...
*Table 3: Average token consumption per task (5 runs). LF-native and LF-unified both beat MCP on every task. LF-manifest is within +2–8% of MCP (consistently worse). The original bash backend is 2.5–5× more expensive on completing runs (up to 8× on near-timeout runs) and excluded from this table for space.*
...
The original bash backend (`livefolders`, not shown in Table 3) is 155–400% more expensive than MCP on tasks that complete (2.5–5× total); runs that approach the 10-turn safety cap push the ratio toward the 8× upper bound cited in Table 3. It generates multi-step bash pipelines in output, consumes full tool-result text in subsequent turns, and sometimes fails to complete within the safety cap. It is not suitable for production token-sensitive deployments.
...
Several factors bound the generalisability of our findings. First, the token efficiency benchmark uses a single model (claude-haiku-4-5) and a single upstream API (mockapi.io, 50 users); token margins may shift for larger models, denser APIs, or workloads with different tool-call-to-output ratios. Second, T2 and T3 assessments capture a point-in-time code and documentation snapshot; rapidly evolving projects may have addressed noted deficiencies since inspection. Third, with only 5 runs per task, the benchmark does not support formal statistical inference; any effect below ~3% should be treated as indicative. Fourth, the evaluation host (Claude Code on Linux x86\_64 with shell access) may not represent cloud-hosted agent deployments where the LLM has no filesystem access, which would lower the LLM compatibility ratings for all os-coupled systems.
...
The most striking finding from our Tier 2 assessment is the gap between claimed and implemented behavior: ToolFS's WASM sandbox was an unimplemented stub at the time of code inspection (early 2025); llm9p's unauthenticated TCP transport creates network-accessible tool surfaces. Practitioners evaluating these systems should treat published claims about security properties as hypotheses pending independent verification against a current release.
...
The author is the primary developer of LiveFoldersFS, one of the two T1 systems evaluated in this paper. The use of explicit evidence tiers and publicly reproducible experiments is intended to mitigate, though not eliminate, the risk of confirmation bias. All T1 results are reproducible from the repository root via `bash research/run-all.sh`.
